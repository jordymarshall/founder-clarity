...
import { DndContext, closestCenter, KeyboardSensor, PointerSensor, useSensor, useSensors, DragEndEvent, DragStartEvent, DragOverlay, DragOverEvent, useDroppable } from '@dnd-kit/core';
...
function SortableInsightBlock({
  ...
  return <div ref={setNodeRef} style={style} className={cn("group transition-all duration-200", isDragging && "opacity-50 rotate-1 scale-105")}>
      <Card className={cn("relative transition-all duration-200 hover:shadow-md cursor-pointer mb-3", config.color, isDragging && "shadow-xl", isEditing && "ring-2 ring-primary ring-offset-1")} role="button" onClick={() => {
      if (!isEditing) onOpenDetails(block);
          ...
          e.stopPropagation();
          handleStartEditing();
        }} className="h-6 w-6 p-0" aria-label="Edit insight">
              <Edit2 className="h-3 w-3" />
            </Button>
            <div className="cursor-grab active:cursor-grabbing p-1" {...attributes} {...listeners}>
              <GripVertical className="h-4 w-4 text-muted-foreground" />
            </div>
          </div>
        
        {isEditing && <CardHeader className="pb-2">
        ...
        <CardContent className={cn(isEditing ? "pt-0" : "pt-3")}>
          {isEditing ? <Textarea value={content} onChange={e => setContent(e.target.value)} placeholder="Add your insights here..." className="min-h-[60px] text-sm resize-none border-none p-0 focus-visible:ring-0 bg-transparent" onKeyDown={handleKeyDown} /> : <div className={cn("min-h-[60px] text-sm leading-relaxed whitespace-pre-wrap break-words", isEmpty ? "text-muted-foreground italic" : "font-semibold hover:bg-background/50 rounded p-1 -m-1 transition-colors")}>
...
function CategoryContainer({
  ...
  return <Card ref={setNodeRef} className={cn("h-full transition-all duration-200", isSubsection ? "min-h-[150px] border-dashed" : "min-h-[300px]", isOver && "ring-2 ring-primary ring-offset-2")}>
      <CardHeader className={cn("pb-3", isSubsection && "pb-2")}>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className={cn("font-semibold", isSubsection ? "text-sm" : "text-base")}>
              {config.label}
            </CardTitle>
            <p className={cn("text-muted-foreground mt-1", isSubsection ? "text-xs" : "text-xs")}>
              {config.description}
            </p>
          </div>
          <Button size="sm" variant="outline" onClick={() => onAddBlock(category)} disabled={category === 'job-to-be-done' && categoryBlocks.length >= 1} className={cn("p-0", isSubsection ? "h-6 w-6" : "h-7 w-7")}>
      ...
      <CardContent className={cn("pt-0", isSubsection && "pb-3")}>
        <SortableContext items={categoryBlocks.map(b => b.id)} strategy={verticalListSortingStrategy}>
          <div className="space-y-3">
            {categoryBlocks.length === 0 ? <div className={cn("border-2 border-dashed border-muted-foreground/20 rounded-lg flex items-center justify-center", isSubsection ? "min-h-[60px]" : "min-h-[100px]")}>
                <p className="text-sm text-muted-foreground">Drop insights here or click + to add</p>
              </div> : categoryBlocks.map(block => <SortableInsightBlock key={block.id} block={block} onEdit={onEdit} onSave={onSave} onCancel={onCancel} editingId={editingId} onOpenDetails={onOpenDetails} />)}
          </div>
        </SortableContext>
      </CardContent>
    </Card>;
...
function ParentCategoryContainer({
  ...
  return <Card ref={setNodeRef} className={cn("h-full min-h-[400px] transition-all duration-200", isOver && "ring-2 ring-primary ring-offset-2")}>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" variant="outline" onClick={() => onAddBlock(category)} className="h-7 w-7 p-0">
      ...
      <CardContent className="pt-0 space-y-4">
        ...
        <SortableContext items={categoryBlocks.map(b => b.id)} strategy={verticalListSortingStrategy}>
          <div className="space-y-3">
            {categoryBlocks.length === 0 ? <div className="min-h-[120px] border-2 border-dashed border-muted-foreground/20 rounded-lg flex items-center justify-center">
                <p className="text-sm text-muted-foreground">Drop insights here or click + to add</p>
              </div> : categoryBlocks.map(block => <SortableInsightBlock key={block.id} block={block} onEdit={onEdit} onSave={onSave} onCancel={onCancel} editingId={editingId} onOpenDetails={onOpenDetails} />)}
          </div>
        </SortableContext>
        ...
        {/* Subsection */}
        <div className="pt-2">
          <CategoryContainer category={subsectionCategory} blocks={blocks} onAddBlock={onAddBlock} onEdit={onEdit} onSave={onSave} onCancel={onCancel} editingId={editingId} isSubsection={true} onOpenDetails={onOpenDetails} />
        </div>
      </CardContent>
    </Card>;
}
export function DeconstructCanvas({
  className,
  idea,
  initialData,
  onBlocksChange
}: DeconstructCanvasProps) {
  ...
  React.useEffect(() => {
    ...
    const seeded = [...push(initialData.problem, 'problem'), ...push(initialData.existingAlternatives, 'alternatives'), ...push(initialData.customerSegments, 'segments'), ...push(initialData.earlyAdopters, 'early-adopters'), ...push((initialData.jobToBeDone || []).slice(0, 1), 'job-to-be-done')];
  ...
  React.useEffect(() => {
    ...
    supabase.functions.invoke('enrich-insight', {
      ...
      if (error) {
        ...
        setDetailData({
          rationale: 'This insight matters because it affects customer progress in this area. Expand with specific context and Customer Forces (push, pull, inertia, friction).',
          sources: [],
          structure: undefined
        });
      } else {
  ...
  const handleDragOver = useCallback((event: DragOverEvent) => {
    ...
    if (Object.keys(categoryConfig).includes(overId)) {
      ...
      if (activeBlock.category !== newCategory) {
        setBlocks(prev => prev.map(block => block.id === activeId ? {
          ...block,
  ...
  return <div className={cn("space-y-6", className)}>
      ...
      {isLoading ? <div className="space-y-6 animate-fade-in">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <ParentCategoryContainer category="problem" subsectionCategory="alternatives" blocks={blocks} onAddBlock={addNewBlock} onEdit={handleEdit} onSave={handleSave} onCancel={handleCancel} editingId={editingId} onOpenDetails={b => setDetailBlock(b)} />

              {/* Customer Segments with nested Early Adopters */}
              <ParentCategoryContainer category="segments" subsectionCategory="early-adopters" blocks={blocks} onAddBlock={addNewBlock} onEdit={handleEdit} onSave={handleSave} onCancel={handleCancel} editingId={editingId} onOpenDetails={b => setDetailBlock(b)} />
            </div>

            {/* Row 2: Full-width Job to be Done section */}
            <div className="w-full">
              <CategoryContainer category="job-to-be-done" blocks={blocks} onAddBlock={addNewBlock} onEdit={handleEdit} onSave={handleSave} onCancel={handleCancel} editingId={editingId} onOpenDetails={b => setDetailBlock(b)} />
            </div>
          </div>

          <DragOverlay>
            {activeBlock ? <Card className={cn("rotate-3 shadow-2xl opacity-90", categoryConfig[activeBlock.category].color)}>
      ...
      {/* Details Dialog */}
      <Dialog open={!!detailBlock} onOpenChange={open => {
      if (!open) {
        setDetailBlock(null);
        setDetailData(null);
        setDetailLoading(false);
        ...
        <DialogContent className="w-[min(96vw,1100px)] max-w-none p-0 overflow-hidden animate-fade-in">
          ...
          <div className="max-h-[75vh] overflow-y-auto px-6 py-5 space-y-5">
            ...
            {detailData?.structure?.evidence && detailData.structure.evidence.length > 0 && <section>
                ...
                <ul className="space-y-2">
                  {detailData.structure.evidence.map((evi, i) => <li key={i} className="text-sm">
                      <div className="text-muted-foreground">{evi.text}</div>
                      <div className="flex flex-wrap gap-2 mt-1">
                        {(evi.urls || []).map((u, j) => <a key={j} href={u} target="_blank" rel="noreferrer" className="text-xs underline underline-offset-2">
            ...
            {detailData?.structure?.implications && detailData.structure.implications.length > 0 && <section>
                <h4 className="text-sm font-medium mb-2">Implications</h4>
                <ul className="list-disc pl-5 space-y-1">
                  {detailData.structure.implications.map((t, i) => <li key={i} className="text-sm text-muted-foreground">{t}</li>)}
                </ul>
              </section>}

            {detailData?.structure?.conclusion && <section>
          ...
          <DialogFooter className="sticky bottom-0 z-10 border-t bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 px-6 py-3">
            {detailBlock && <Button variant="outline" onClick={() => {
